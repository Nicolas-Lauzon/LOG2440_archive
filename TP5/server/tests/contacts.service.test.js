...
describe('ContactsService tests', () => {
  ...
  it('should generate a new id on a duplicate Contact id', async () => {
    const contact1 = { id: 1, name: 'Test', email: 'a@b.ca', message: 'test' };
    const contact2 = { id: 2, name: 'Test', email: 'a@b.ca', message: 'test' };
    const contact3 = { id: 2, name: 'Test', email: 'a@b.ca', message: 'test' };
    await service.addNewContact(contact1);
    await service.addNewContact(contact2);
    await service.deleteContactById(contact1.id);
    await service.addNewContact(contact3);
    const allContacts = await service.getAllContacts();
    expect(allContacts[1].id).toEqual(1);
  });
